...
class EmployeeLotsRepositoryTest {
    ...
    void when_employeeLot_attempt_to_save() {
        EmployeeTechnology persistedEmployeeWithTechnology = employeeWithTechnologyStubFactory.supplyEmployeeWithTechnology(
        ...
        Employee employeeWithLots = employeeRepository.findAll().stream()
                .findFirst().orElseThrow(() -> new RuntimeException("Сотрудник с лотами отсутствует"));
        assertAll("Проверка сохранения тестовой записи",
